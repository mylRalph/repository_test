...
os.environ["KMP_DUPLICATE_LIB_OK"]="TRUE"  # 此语句为发生报错后按CSDN添加的
...
base_lr = 0.01
lr_decay_epoch = 5
dev = 'cuda:0'  or "cuda:1" or "cpu"
work_dir = './trained_models' #训练好的模型
...
def preprocess_data(pra_data, pra_rescale_xy):
	...
	feature_id = [3, 4, 9, 10]
	ori_data = pra_data[:,feature_id].detach()#只取下了x,y以及方向和mask
	data = ori_data.detach().clone()

	new_mask = (data[:, :2, 1:]!=0) * (data[:, :2, :-1]!=0)  # *是按位置相乘, 这里先用!=转换成布尔值再相乘(针对xy坐标)   [注意]这里用xy坐标作为衡量指标,”1”与”-1”两块切片形成前后对应,但凡有一个xy坐标不存在(即无法计算速度),相乘的结果就为False
	data[:, :2, 1:] = (data[:, :2, 1:] - data[:, :2, :-1]).float() * new_mask.float() # 算速度
	data[:, :2, 0] = 0
	...
	data[:,:2] = data[:,:2] / pra_rescale_xy  # 个人理解为pra_rescale_xy(1,2,1,1)中的”两个1”分别用于除xy方向上的两个速度

	return data, ori_data, object_type # data已经是转换为速度的”新数据”,ori_data是仅取下x/y/方向/标记的数据, object_type是物体类型


def compute_RMSE(pra_pred, pra_GT, pra_mask, pra_error_order=2):
	pred = pra_pred * pra_mask # (N, C, T, V)=(N, 2, 6, 120)    遮掉无效预测
	GT = pra_GT * pra_mask # (N, C, T, V)=(N, 2, 6, 120)    # ground true值
	x2y2 = torch.sum(torch.abs(pred - GT)**pra_error_order, dim=1) # x^2+y^2, (N, C, T, V)->(N, T, V)=(N, 6, 120)    这里的sum将x和y方向上的速度误差加和
	overall_sum_time = x2y2.sum(dim=-1) # (N, T, V) -> (N, T)=(N, 6)     将某个时间步中所有物体的误差(已经包括了xy方向上的速度误差)加和
	overall_mask = pra_mask.sum(dim=1).sum(dim=-1) # (N, C, T, V) -> (N, T)=(N, 6)
	overall_num = overall_mask  # 统计出每个时间步的有效物体数
...
def train_model(pra_model, pra_data_loader, pra_optimizer, pra_epoch_log):
	# pra_model.to(dev)
	pra_model.train() # pra_model.train()语句的作用是将模型切换到训练模式，以便在训练过程中启用特定于训练的层行为。在进行模型评估时，记得使用pra_model.eval()将模型切换回评估模式
	rescale_xy = torch.ones((1,2,1,1)).to(dev)  # 用于缩放模型的预测输出，使其与数据集中的坐标尺度一致。这对于评估模型性能和计算损失函数时非常重要
	rescale_xy[:,0] = max_x
	rescale_xy[:,1] = max_y
	...
	for iteration, (ori_data, A, _) in enumerate(pra_data_loader):
		...
		for now_history_frames in range(1, data.shape[-2]): # data第一个frame的xy速度数据无效
			input_data = data[:,:,:now_history_frames,:]  # 使用前now_history_frames个帧作为输入数据 # (N, C, T, V)=(N, 4, 6, 120)
			output_loc_GT = data[:,:2,now_history_frames:,:] # 表示目标位置（x和y坐标）在每个时间步和节点上的值。我们从now_history_frames时间步开始提取数据，直到最后一个时间步 # (N, C, T, V)=(N, 2, 6, 120)
			output_mask = data[:,-1:,now_history_frames:,:] # 用于从原始数据中提取掩码,掩码用于在训练和评估过程中忽略无效数据 # (N, C, T, V)=(N, 1, 6, 120)   # 取出data中的有效性标记

			A = A.float().to(dev)  # A在xin_feeder_baidu.py中变成了(8[batch大小,可变],3,120,120) "3"代表了0跳、1跳、2跳
...
def run_trainval(pra_model, pra_traindata_path, pra_testdata_path):
	...
	loader_test = data_loader(pra_testdata_path, pra_batch_size=batch_size_test, pra_shuffle=True, pra_drop_last=True, train_val_test='all')
	...
	for now_epoch in range(total_epoch):
		all_loader_train = itertools.chain(loader_train, loader_test) # 将训练集和测试集合并，一起用于训练模型。这可以增加模型的训练数据量，从而提高模型的性能
...
if __name__ == '__main__':
	graph_args={'max_hop':2, 'num_node':120} # 设置最大跳数和图中的结点数量
